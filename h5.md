...
In our sceraio Alice will send an encrypted message to Aha.
...
The I checked 4 first rows of the public key:
...
The public key in ASCII armor and not readable.

#### Alice, Simulated

I created folder in our homedirectory (Alice) and gave it rights so we can only use it.
...
To simulate Alice, we can just work inside this folder, and replace 'gpg' with 'gpg --homedir .' Every command as Alice will run in Alice's directory 'cd ~/alice', and start with 'gpg --homedir.
...
Like we did it wit Aha, this time we need to use the "gpg --homedir" refering to Alice.
